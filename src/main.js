...
$(document).ready(function() {
  ...
  $('#doctorForm').submit(function(event) {
    ...
    promise.then(function (response) {
      ...
      $('.doctorsList').text(`Doctors named "${searchDoctor}" in your area :`);
      if(body.data.length === 0){
        $('.doctorsList').text(`Sorry, there are no results for your search.`)
      } else {
        for (let i = 0; i < body.data.length; i++) {
          $('.doctorsList').append(`<h2><br>${body.data[i].profile.first_name} `+`
            ${body.data[i].profile.last_name}`+`<br></h2><br><h4>Gender: </h4> 
            ${body.data[i].profile.gender}`+` <br> <h4>Phone number:</h4>
      ...
      if(body.data.length === 0){
        ...
        for (let i = 0; i < body.data.length; i++) {
          $('.doctorsList').append(`<h2><br>${body.data[i].profile.first_name} `+`
            ...
            ${body.data[i].specialties[0].description}`+`<br><h4>Title: </h4>`+`
